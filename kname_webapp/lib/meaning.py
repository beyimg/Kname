...
7. 영어 원래 이름과의 다리 — 발음이 비슷한지, 글로벌하게 통하는지만 (해당될 때만).
   ※ 이름의 뜻은 오직 한자에서만 나옵니다. 영어 이름 자체의 뜻을 한국 이름의 뜻에
     섞지 마세요. 예를 들어 영어 이름이 Rosa여도, 한자에 '장미'가 없다면 '장미처럼
     아름다운'이라고 쓰면 안 됩니다. 영어 이름은 발음·친숙함을 말할 때만 언급합니다.
...
- 영어 답변은 반드시 "한글이름 (한자, 로마자)" 형식으로 시작하세요.
  예: "민수 (旻秀, Minsu) joins ..." / 한자가 없는 우리말 이름이면 "마루 (Maru) — ...".
  로마자만 쓴 "Minsu carries ..." 같은 도입부는 안 됩니다.
...
6. 이 이름의 성별 사용 경향
7. 영어 원래 이름과의 다리 — 발음이 비슷한지만 (해당될 때만).
   ※ 이름의 뜻은 오직 우리말 낱말의 뜻에서만 나옵니다. 영어 이름 자체의 뜻을
     한국 이름의 뜻에 섞지 마세요. 영어 이름은 발음·친숙함을 말할 때만 언급합니다.
8. (선택) 확실히 아는 경우에만 문화적 레퍼런스 추가. 불확실하면 절대 추가하지 마세요.

...
class KoreanNameEngine:
    ...
    # 발음이 정확히 일치하는 성씨에 주는 보너스.
    #
    # 아래 식은 인구(0.75)를 발음(0.20)보다 훨씬 크게 반영한다. 한국 성씨는
    # 김·이·박이 44%라, 발음만 따르면 실제로 거의 없는 성씨가 자주 나오기
    # 때문이다. 그런데 그 균형이 **정확 일치까지 뒤집었다**.
    #
    #   Antonelli → 안토넬리
    #     이: 발음 불일치(sim 2) · 730만명 → 0.8824   ← 이쪽이 뽑혔다
    #     안: 발음 정확(sim 0)   ·  68만명 → 0.8727
    #
    # 발음이 어긋날 때(sim 0→2) 잃는 점수는 0.100인데, 인구가 68만→730만으로
    # 늘면 0.110을 얻는다. 인구 이득이 발음 감점보다 커서 정확 일치가 밀렸다.
    # 사용자에게는 "발음이 딱 맞는 성씨가 있는데 왜 다른 걸 줬나"로 보인다.
    #
    # 보너스 0.12는 그 역전을 막을 최소값이다(0.110보다 조금 크다).
    # 가중치 자체를 바꾸는 방법(발음 0.35 / 인구 0.60)과 비교해 보니 248개
    # 성씨 중 247개가 같은 결과였고, 다른 하나(Zhu)는 이쪽이 더 맞았다.
    # 그래서 영향 범위가 좁은 보너스 방식을 택했다.
    EXACT_SURNAME_BONUS = 0.12
    ...
    def _ls(self, f, s):
        ...
        return (
            ...
            0.05 +
            (self.EXACT_SURNAME_BONUS if s == 0 else 0.0)
        )

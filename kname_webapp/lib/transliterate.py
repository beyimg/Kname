...
class Transliterator:
    ...
    # 된소리 음절 차단 필터(_BAD_SYLLABLE)는 제거했다.
    #
    # 원래 의도는 "외래어 표기법은 된소리를 쓰지 않는다"를 강제하는 것이었으나,
    # 실제로는 된소리 음절 2,940개 중 8개(0.27%)만 막고 있었다. 짜·쩌·따·뽀·꼬는
    # 그대로 통과했으므로 규칙을 강제하지도 못했다.
    #
    # 반면 그 8개 중 6개는 특정 언어의 정규 표기였다.
    #   쓰 — 일본어 つ  (쓰나미·미쓰비시 / Natsuki → 나쓰키)
    #   쯔 — 중국어 zi  (라오쯔·쿵쯔 / Ziyi → 쯔이)
    #   뻐·껴·똑·빡 — 태국어 ป·ก·ต 계열 (푸껫·끄라비)
    # 즉 이득은 거의 없고, 정답을 걷어내는 부작용만 있었다. 한 글자씩 예외를
    # 추가하는 방식으로는 언어가 늘어날 때마다 같은 사고가 반복된다.
    #
    # 된소리를 피하라는 지침은 _GUIDE(프롬프트)에 남아 있고, 명백히 깨진 출력은
    # _plausible 의 길이 검사가 걸러낸다. LLM이 이상한 답을 내도 규칙 기반
    # 폴백이 받아주므로, 하드 필터로 정답을 버리는 것보다 통과시키는 편이 낫다.
